...
}.data.reduce((map, { value, timestamp }) => {
  const date = dayjs.utc(Number(timestamp) * 1000);

  if (!date.isValid()) {
    throw new Error('Invalid date');
  }

  map.set(date.toISOString(), Number(value));
